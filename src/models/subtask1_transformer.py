...
class Subtask1Regressor(nn.Module):
...
def save_hf_checkpoint(model: Subtask1Regressor, tokenizer, out_dir: str | Path) -> Path:
    ...
    config.classifier_dropout = getattr(model.dropout, "p", 0.1)
    config.to_json_file(out_dir / CONFIG_NAME)
...
def load_hf_checkpoint(
    ...
    model = Subtask1Regressor(model_name=model_name, dropout=dropout)
    state_dict = torch.load(out_dir / WEIGHTS_NAME, map_location="cpu")
